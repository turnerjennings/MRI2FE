...
def validate_registration(registered):
    ...
    assert "xi" in registered, (
        "Missing damping ratio (xi) in registered images"
    )
    ...
    assert registered["mu"].shape == registered["xi"].shape, (
        "mu and xi should have same dimensions"
    )
...
def validate_material_constants(material_constants):
    ...
    for key in required_keys:
        assert key in material_constants, (
            f"Missing {key} in material constants"
        )
...
def validate_mesh_data(mesh_data):
    ...
    assert mesh_data["element_table"].shape[1] == 12, (
        "Element table should have 12 columns"
    )
    assert mesh_data["node_table"].shape[1] == 4, (
        "Node table should have 4 columns"
    )
    assert mesh_data["centroids"].shape[1] == 3, (
        "Centroids should have 3 columns"
    )
    ...
    assert np.all(mesh_data["centroids"] >= min_coords), (
        "Centroids should be within node bounds"
    )
    assert np.all(mesh_data["centroids"] <= max_coords), (
        "Centroids should be within node bounds"
    )
...
def validate_fe_model(fe_model):
    ...
    for element in element_table:
        ...
        assert np.all(active_nodes > 0), (
            "Active node references should be positive"
        )
        assert np.all(active_nodes <= max_node_id), (
            "Active node references should be valid"
        )

        if is_tet4:
            assert np.all(node_refs[4:] == 0), (
...
def test_mre_pipeline():
    ...
    mesh_data = model.calculate_element_centroids(mesh_path=mesh_path)
